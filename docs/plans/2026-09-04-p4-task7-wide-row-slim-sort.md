...
Status: plan (DRAFT — pending Codex plan-gate). Author: Opus, 2026-09-04.
...
SELECT projection that reaches the sorter. Provide the slim projection as a
reorder-specific path; keep the legacy full projection only where the shim / EXPLAIN
/ diagnostic APIs still require it (they are not on the sorter path).
...
- the raw child slice from a NEW `SpillChildKeys` reader/cursor (row_nr +
  fk_join_key + src_{idx}), exposed for phase 3 with the SAME row-number
  alignment assertions `JoinRowCursor` already enforces (a lost/misaligned suffix
  fails closed, never silently self-validates).
`resolve_batch` is refactored to take BOTH slices: `_batch_remap_values` and
`_append_output_batch` read the raw components from the child slice and the
match/masked-parent values from the slim slice. The payload-store batch boundary
(source-chunk granularity) is preserved EXACTLY, so output row/chunk order is
byte-identical to today and to `_batch_join`.
...
bound on the actual slim sortable row's masked-parent-component byte width,
including Arrow structural overhead (offsets, validity, fixed widths). Rules
(fixing the current `_key_width.py` gaps dennis/Codex flagged):
...
envelope for the sorter path. Raw child values NO LONGER affect sort-run or
merge-head size (they never enter the sorter). Wide raw values can still transit
phase-1 staging and phase-3 Arrow batches (unavoidable — `_batch_join` has the
same O(batch) transit); that is covered by end-to-end RSS tests, not the sorter
envelope. Truly-wide MASKED payloads fall back before the source is consumed. Do
...
- Same orphan under REMAP, DROP (if supported), FAIL (fail-closed, per the §6.1
  exception carve-out where applicable).
- Dictionary-encoded raw string AND binary keys, matched and orphaned.
- Composite key with one very wide raw component.
- Multiple + overlapping incoming edges.
...
- Residual-bound boundary: cap−1 (reorder), cap (fall back), cap+1 (fall back).
...
- Existing 1.35× single-edge sorter RSS proof + the multi-edge RSS proof (T12)
  stay green.
- NEW end-to-end RSS cases with one and several multi-MB raw values (peak RSS
  within the envelope — proving raw width no longer drives sorter residency).
