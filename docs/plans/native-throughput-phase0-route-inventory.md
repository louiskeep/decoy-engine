...
| 5b | Polars execution adapter | `PolarsExecutionAdapter` via `select_execution_adapter` | Dormant - explicit `substrate="polars"` opt-in only; the S13 polars default was reverted, `resolve_substrate` defaults to pandas |
...
- **Substrate resolution:** `resolve_substrate` `src/decoy_engine/execution/_substrate.py`; default `_DEFAULT_SUBSTRATE = "pandas"` (the S13 polars default was reverted; polars is now a dormant explicit opt-in).
...
| Polars adapter (5b) | **dormant / explicit opt-in only** | `resolve_substrate` default is `"pandas"` (the S13 polars default was reverted), so a bare engine call selects pandas; polars runs only on an explicit `substrate="polars"` / `DECOY_SUBSTRATE=polars`. Platform never passes substrate (builds `PandasExecutionAdapter` directly) and CLI forwards substrate only on `--chunked`. FK/composite fall back to pandas oracle. Kept for subset FK joins; full removal deferred. |
