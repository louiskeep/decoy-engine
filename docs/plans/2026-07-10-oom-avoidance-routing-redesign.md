...
**Recalibration trigger (documented in `_mem_estimate.py`; recompute via the harness + re-pin):** (1) **drift** -- B5 telemetry (`_mem_telemetry.recalibrate_k`) sees an isolated job whose INTERCEPT-REMOVED `observed_slope` (see the TB-5 precondition #73 note below) exceeds `current_k` -> RAISE immediately (safety), or the max `observed_slope` over >= `min_samples_for_lower` isolated jobs falls below `current_k / (1 + error_band)` -> LOWER (gated); (2) **dependency/route change** -- a pyarrow/DuckDB/pandas major bump or a route buffering change invalidates the slope+intercept; (3) **new schema class** -- a production shape unlike pooled/numeric/unique (wide-binary, nested).
...
### 13.2 TB-5 precondition -- fixed INTERCEPT term added (2026-07-13, issue #72)

The dennis gate on TB-4 flagged a real small-size gap: TB-4 pinned MEASURED per-route slopes but kept a **through-origin** model (`predicted = basis * slope`), which OMITS the fixed baseline RSS (interpreter + pyarrow + DuckDB) and therefore UNDER-predicts SMALL-basis jobs -- the OOM-unsafe direction. Example: `numeric_fk` full_frame @500k predicted **767 MB** (191.7 MB basis * 4.0) versus a real **858 MB** peak, -11%. This must close before TB-5 flips `use_byte_estimate_routing` / the governor default-ON.

**Root fix -- the model is now `predicted = intercept + basis * slope`.** From the same two committed TB-4 points per route, both terms fall out: `slope = d(peak)/d(basis)`, `intercept = peak - slope*basis`. The two-point fit from the numeric_fk shape (no string pooling -> the worst-case per-byte slope; `raw_data_bytes` prices it exactly) is:

| route | measured slope | measured intercept | pinned slope | pinned intercept |
| --- | --- | --- | --- | --- |
| `full_frame` | 3.45 | ~197 MB | **4.0** (unchanged) | **200 MiB** |
| `out_of_core` | 0.95 | ~447 MB | **1.5** (unchanged) | **450 MiB** |
| `sequential` | 3.28 | ~172 MB | **4.0** (unchanged) | **200 MiB** |

- **The SLOPES are the TB-4 values, kept UNCHANGED.** Each already exceeds its measured two-point slope, so with a strictly positive intercept `intercept + basis*slope` is **>= the old `basis*slope` at EVERY size** -- the fix RAISES the omitted small-basis floor without lowering any prediction. The three `K_*_COLD_START` constants were **renamed to `K_FULL_FRAME_SLOPE` / `K_OUT_OF_CORE_SLOPE` / `K_SEQUENTIAL_SLOPE`** to reflect that they are pure slopes now, not combined intercept+slope multipliers.
- **The INTERCEPT is pinned PER ROUTE** (`K_INTERCEPT_BYTES = 200 MiB` for the in-core routes full_frame/sequential; `K_OUT_OF_CORE_INTERCEPT_BYTES = 450 MiB`) because it differs materially: out_of_core (~2.3x) runs DuckDB and holds a budget-bounded working buffer on top of the interpreter/pyarrow floor the in-core routes share. A single shared 200 MB would under-predict out_of_core (unsafe); a single shared 450 MB would over-inflate the in-core routes' small jobs by ~250 MB. Each pinned value is the measured intercept rounded UP (conservative-filter ruling: over-predict, never under).
- **Acceptance (`tests/unit/execution/test_mem_calibration.py`).** Proven fail-pre / pass-post on the canonical point (numeric_fk full_frame @500k: through-origin 767 < 858; intercept model 976 >= 858), plus **predicted >= observed at BOTH the small AND large measured points for EVERY route** (all 16 sweep points), the intercept never lowering a prediction, and the slopes still within `K_CALIBRATION_ERROR_BAND` above the measured worst-case slope. out_of_core is now conservative at its measured points too (its ~450 MB floor no longer omitted), while the runtime budget + governor (TB-1/TB-2/TB-3) remain its real RUNTIME bound.
- **No live routing change.** This is a values/model change behind the flag-gated estimate path; `use_byte_estimate_routing` stays default-**False** (it flips at TB-5, not here). Golden test-flight unchanged (53/53 + fingerprint 5/5) -- the model feeds the routing estimate only, flags off, so determinism is unaffected.

#### TB-5 precondition #73 -- the drift detector must be intercept-aware too

Adding the intercept to the estimator (above) left a matching bug in the B5 drift detector, which the #72 dennis gate flagged and #73 closes before TB-5 wires the drift loop. `recalibrate_k` (`_mem_telemetry.py`) still computed `observed_k = actual_peak_bytes / raw_bytes` -- a **through-origin point ratio** -- and compared it against `current_k`, now a pure `K_<route>_SLOPE`. With the estimate carrying an intercept, that point ratio is STRUCTURALLY inflated at small basis: a job that FITS the model exactly reads a ratio above the slope (numeric_fk full_frame @500k: 858/191.7 = **4.48 > 4.0**), so the detector would spuriously fire a raise on every small job and ratchet the slope upward on nothing.

- **Root fix -- aggregate an INTERCEPT-REMOVED slope.** `MemoryTelemetryRecord.observed_slope = max(0, (actual_peak_bytes - route intercept) / raw_bytes)` removes the SAME per-route intercept the estimator adds (via a new `route_intercept_bytes` accessor in `_mem_estimate.py`, the single source of truth), and `recalibrate_k` compares THAT against the pinned slope -- slope-vs-slope, dimensionally consistent (the `_K_FLOOR_DEFAULT` values were already in slope units). The max-aggregation and asymmetric raise/lower gates are unchanged; `observed_k` is kept as a diagnostic only. Chose the slope-subtraction form over a predicted-vs-residual comparison because it preserves the module's entire safety architecture (max-of-slopes, floor, gates) and only corrects the quantity aggregated.
- **Direction stays safe.** A raise still fires immediately on any genuine slope drift (peak growing faster per byte than the pinned slope, intercept removed); only the intercept-inflation false positive is removed. numeric_fk @500k: point ratio 4.48 > 4.0 (old, spurious) vs intercept-removed slope 3.43 < 4.0 (new, correct hold).
- **Acceptance (`tests/unit/execution/test_mem_telemetry.py`).** Fail-pre / pass-post: a small-basis fitting job's point ratio would have raised, its intercept-removed slope does not, and a pool of 50 such jobs never ratchets the slope up; a genuine slope drift beyond `K_CALIBRATION_ERROR_BAND` STILL raises; route-awareness pinned (out_of_core removes its own 450 MiB intercept). Flags stay default-OFF; the drift loop is unwired/platform-owned, so this is a values/logic fix behind it. Golden test-flight unchanged (telemetry/drift is off the determinism path).
