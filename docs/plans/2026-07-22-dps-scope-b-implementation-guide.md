...
| C-B5, categorical labels unstable under adjacency (new with round 6; the only defect in this program that broke the GUARANTEE, not a test or an error contract) | A label is a function of the VALUE, not the column's storage width: `_canonical_label` renders an integral real as its integer string, so an int/float upcast cannot move it | `test_categorical_labels_are_stable_when_a_null_upcasts_the_column` and `test_canonical_label_merges_integral_reals_but_preserves_everything_else` | pandas upcasts an integer column to float64 the moment a null enters it, so under plain `str()` adding ONE row replaced EVERY label (`ints 0..7` -> `["0".."7"]`, plus a null -> `["0.0".."7.0"]`). The multisets differed by 2n elements while the grouped-count measurement certifies `map(1)`, understating true sensitivity by a factor of the column's cardinality. Normalization was recordwise GIVEN A FRAME, but a frame's dtype is a function of ALL its rows. The fixtures are INTEGER on purpose: every recordwise test before this one used strings, which never upcast, which is why five rounds missed it. The second test pins that the canonicalization stays narrow (bool excluded, non-finite falls through, 7.5 untouched). |
