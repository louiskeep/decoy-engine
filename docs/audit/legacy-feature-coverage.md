...
| M3 | Large-file chunked processing (>1 GB threshold, configurable via `large_file_threshold_gb`) | `masker/masker.py:_process_large_file()`, `internal/large_file_processor.py` | **No — graph source.file materializes the full file in RAM.** Code audit (session 16): `source_file._apply_duckdb()` calls `con.execute(sql).to_arrow_table()`, which fully materializes regardless of file size. DuckDB's query optimizer helps parquet (column projection) but does not provide streaming for CSV `SELECT *`. There is no chunked pipeline in the graph path. | **ACCEPTED LIMITATION (session 16, code audit).** No fixture test required pre-GA. Rationale: (1) no dedicated `LargeFileProcessor` unit tests exist — deletion loses no regression coverage; (2) pre-GA, no external users depend on streaming; (3) parquet is the recommended format for large datasets (DuckDB handles it far more efficiently than CSV); (4) for RAM-constrained CSV use cases, V2.2 backlog item: lazy streaming via Arrow `RecordBatchReader` instead of `to_arrow_table`. Operator doc must note: CSV files >1 GB require ≥4 GB host RAM in graph mode; parquet strongly preferred. |
...
- [x] **M3/L1 large-file verification**: **ACCEPTED LIMITATION — session 16, code audit.** Graph `source.file` materializes all rows to Arrow (no streaming). For large CSV: operator must have ≥4 GB RAM. Parquet strongly preferred. V2.2 backlog: lazy streaming source. Operator guide must document RAM guidance before V2.1 deletion PRs open.
...
Count update (session 16): M3/L1 ACCEPTED LIMITATION. **Remaining actionable items = 0.**
All pre-deletion blockers resolved. Dennis approves proceeding to V2.1 deletion phase. Pre-condition: add operator RAM guidance to deployment docs before the first deletion PR opens (one paragraph in the operator guide; ~10 minutes of work). V2.2 backlog: lazy streaming source for large CSV files.
