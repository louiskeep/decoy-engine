...
- **`native_route_enabled: bool = False`** (new `run_pipeline` kwarg): toggles the native lane on; passed `False` defaults to the existing oracle path; a stringy `"false"` value raises at validation time (fail-early like all other routing knobs).
- **`_native_route.py` (new module):** Static admission gate (`static_candidacy`), single-batch dynamic admission (`peek_and_admit`), route-selection entry point (`maybe_run_native_route`), and ledger/report dataclasses (`NativeStaticCandidacy`, `NativeBatchAdmission`, `NativeRouteLedger`, `NativeRouteReport`). The static gate runs before any source is touched, proving eligibility from config, plan, and routing signals; the dynamic gate opens the source exactly once, taking its first batch as both schema evidence and the first chunk of the single masking pass.
- **`_native_route_exec.py` (new module):** Production execution entry point (`try_native_route`) and stream-processing helpers (`_mask_one_batch`, `_validate_ledger`, `_run_native_streaming`, `_masked_batches`, `_execution_envelope`, `_execution_adapter_stamp`, `_resolve_truncate_keep`, `_resolve_strategy_cfg`). The lane branches after layer-1 FK routing and before source materialization, streams the source in 50k-row batches (`_NATIVE_BATCH_ROWS_DEFAULT`), masks each batch column-by-column via the Python kernel (`native._kernels_scalar`), emits to a transactional sink (streaming `ParquetTransactionalSink` for memory-bounded output or resident return for in-memory), validates the ledger post-pass, and atomically commits; any mid-stream error (kernel failure, schema drift, ledger violation, commit failure) raises and discards the staged artifact. First-batch admission is the only scope check because the three admitted strategies on `utf8` columns produce output types independent of null state: `passthrough` yields `utf8`, and `redact`/`truncate` always yield `string`; a zero-row source reroutes to the oracle before any output (the oracle types `redact`/`truncate` as `double` there, creating a type mismatch).
...
- **Task 4 - bounded-parent split-dedup:** Out-of-core FK relation staging now enforces cardinality ceiling on parent keys via early split-dedup (`_relation.py`). Bounded-parent dedup ensures the parent key relation stays within single-batch memory, closing a pathological-schema DoS where a single parent table with 100M+ distinct child keys could blow relation-staging memory. Fallback policy: if de-duping exceeds 2x single-batch memory, the relation skips dedup and accepts unbounded residency (rare schema, logged for observability).
- **Task 6 + Task 7 - auto-routed reorder driver:** The out-of-core route now auto-selects between the fast-path `_batch_join` route (small-parent, no spill) and the new `_stream_driver` reorder route (large-parent, external-sort-stabilized contiguity) per-table. Selection by parent-key row count via `_route_policy.decide_route` with default threshold 2,000,000 rows (configurable via `out_of_core_reorder_threshold_rows`). The reorder route uses `_external_sort.py` (stable two-file spill) and `_stream_join.py` (streaming-join core, decomposed from oversized `_runner.py`), routing both high-fan-in multi-parent and single-parent large-relation tables to bounded RSS. Task 7 wired the decision live in `_runner.py`'s per-table loop.
- **Polars-hash byte-parity (Phase 4 slice 3):** Polars hash strategy now routes through the shared Arrow kernel (`hash_array` in `kernel/`) instead of Polars-native hash, closing a cross-adapter divergence where Polars Series.to_list() truncates datetime to microseconds while pandas to_arrow() preserves nanoseconds, causing timestamp[ns] keys to hash differently. Parity matrix: byte-identical for string/int/bool/date32/IANA-tz-timestamp/positive-decimal on both adapters; exotic dtypes remain a pre-existing adapter-ingestion divergence (cascade safety refuses them as FK keys). 19-case parity test matrix plus NaN/None handling and tz-robustness. Mutation grading: 40 killed, 0 unresolved on changed path.
- **Cascade safety tightening (Phase 4 slice 4):** Chunked-execution FK self-mask gate narrowed from a permissive list to hash-only plus predicate 12 (exact cross-adapter-safe dtype set, two-stage declared+real guard). Closes two cross-model gate-caught fail-open holes: undeclared-dtype hash key bypass and composite-component parent misclassification. Some previously-admitted non-hash exotic-dtype FK self-masks now fail closed with clear error pointing at `run_pipeline` or `run_sequential` config.
- **HIGH-1 - out-of-core module decomposition (Phase 4 slice 5):** Split oversized `_runner.py` and `_external_sort.py` to meet the module-size sentry. `_stream_join.py` (streaming-join core), `_stream_join_cursors.py` (multi-parent cursor tracking), and route-policy modules split out. Branch CI now passes the sentry; no new exports or public-API change.
- No engine version bump: all slices are correctness/efficiency refinements, output byte-identical to prior art (matching pinned pandas oracle), and the routing default (sequential for FK; chunked auto-routing for non-FK) is unchanged. Pre-GA hard-delete caveat: Task 7's live reorder routing is built but held pre-GA; it may be refined or deferred post-release based on production telemetry.
