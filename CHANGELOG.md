...
- `to_pandas_fk_safe()` + `fk_columns_for_table()`: at ingestion, every relationship-edge parent/child key column routes through pandas' nullable `Int64` extension dtype (via pyarrow's `types_mapper` hook) instead of the float64-on-null default. Every other column is unaffected -- this is a targeted fix for FK referential-integrity data, not a blanket dtype policy change.
- `lossless_fk_int_values()`: at FK write-back, classifies a resolved output column by each value's own type (matching the out-of-core route's matched/orphan split, not `fk_key_value`'s join-key folding) and either builds a nullable `Int64` array (exact, round-trips to Arrow `int64`) or raises the SAME `FK_KEY_DTYPE_UNSUPPORTED_CODE` the out-of-core route already raises for the one genuinely unresolvable shape (a literal float value sharing a column with an integer beyond `2**53`).
...
The chunked/self-masking FK route (`run_mask_pipeline_chunked`, REMAP-only) reuses `PandasExecutionAdapter.run()` per chunk and inherits the write-back fix; its runtime `RelationshipGraph` is intentionally empty (self-masking has no parent-map join), so a `passthrough`-strategy chunked FK edge's OWN ingestion is not covered by this fix -- a narrower, pre-existing gap noted for a future pass, not part of this HIGH's blast radius (chunked FK edges are REMAP-only and every other admitted strategy re-derives its output value rather than preserving the raw key).

**Golden gate**: `scripts/test_flight.py` 53/53 pass; fingerprints unchanged, 5/5. No existing FK/parity fixture carries a key beyond `2**53`, so this fix does not change any existing job's output bytes.

New tests: `tests/unit/execution/test_de10_fk_lossless_typing.py` (parametrized across full_frame/sequential/out_of_core: exact survival beside a null, route parity, and the fail-closed mix all raise the identical code). Updated `tests/parity/test_out_of_core_fk_parity.py::test_matched_float_and_int_orphan_beyond_precision_fails_closed` to assert the pandas oracle now raises the same code instead of silently rounding (was previously pinning the bug this fix closes).
