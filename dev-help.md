# decoy-engine — Developer Reference

## First-time setup

```bash
cd forge-engine                  # directory name still on disk; package name is decoy-engine
pip install -e .                 # installs decoy-engine as editable package
...
```bash
python -c "from decoy_engine import Masker, DataGenerator; print('import OK')"
```

## Adding a new masking transform

1. Create `src/decoy_engine/transforms/your_name.py` implementing `BaseMaskingStrategy`
2. Register it in `src/decoy_engine/transforms/factory.py`
3. Export it in `src/decoy_engine/transforms/__init__.py`
4. Add unit tests in `tests/unit/test_transforms.py`
5. Do NOT add it to `decoy_engine/__init__.__all__` unless it's part of the public API

## Adding a new connector

1. Create `src/decoy_engine/connectors/your_connector.py` subclassing `IOHandler`
2. Register it in `src/decoy_engine/connectors/factory.py`
...
This package has NO dependency on `decoy` (CLI) or `decoy-platform`. If you find yourself importing either, stop — that's an architecture violation.
