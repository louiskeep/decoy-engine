...
``docs/v2/ml/model-card.md`` §7 (band composition table) is the source of
record for per-band counts; ``docs/v2/ml/lightgbm-report.json`` (regenerated
by the golden gate) remains the source of record for the aggregate metrics
and reliability figures it actually contains. The qualitative shape observed
to date:

    BAND MEASUREMENTS (lgbm-v1; see model-card.md §7 for exact counts):
    - high   (>= 0.95): now populated and reliable.  On the expanded (2958-col)
                         ...
                         band, measured at 541/592 = 100% accuracy (Wilson
                         lower 0.993).  The >=0.95 floor is reachable.
    ...
    - low    (op-thresh to 0.70): the shipped pack measured ZERO false
                           positives; residual errors in this range are
                           cross-type FALSE NEGATIVES (mrn/health_plan_id
                           confusion) on cryptic-header columns, not
    ...
    isotonic calibration, which the shipped pack showed strictly dominates
    sigmoid on this corpus (mean calibration error 0.0536 vs 0.097).
