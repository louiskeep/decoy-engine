...
class DataGenerator:
    ...
    def __init__(self, config_path: str, logger=None, ctx=None):
        ...
        # only holds for mask.
        self.pipeline_derive_key = ctx.pipeline_derive_key if ctx is not None else None
        ...
        self.column_generator = ColumnGenerator(
            self.seed,
            self.logger,
            derive_key=self.pipeline_derive_key,
    ...
    def generate(self):
        ...
        self.logger.info(
            f"Generating {len(tables)} tables in the order specified in the configuration"
        )
    ...
    def _generate_table(self, table_config: dict[str, Any]):
        ...
        if not output_path:
            output_dir = self.config.get("generator_settings", {}).get(
        ...
        progress = ProgressLogger(
            self.logger, len(column_configs), f"Generating columns for {table_name}"
        )
        progress.start()
        ...
        for column_config in column_configs:
            ...
            if column_name and data_type:
                ...
                df[column_name] = self.column_generator.generate_column(
                    num_rows, column_config, table_name, self.reference_data
                )
                progress.update(1)
            else:
        ...
        if output_type == "csv":
            ...
            if not definition_path:
                ...
                raise ValueError(
                    "Fixed-width output requires a definition_path in fixed_width_options"
                )
    ...
    def _process_referenced_formulas(self):
        ...
        for table_config in tables:
            ...
            for col_config in table_config.get("columns", []):
                if col_config.get("type") == "formula" and col_config.get("references"):
                    has_referenced = True
            ...
            try:
                ...
                if output_type == "fixed_width":
                    ...
                    self.logger.debug(
                        f"Processing fixed-width file for composite formulas: {output_path}"
                    )
                    ...
                    if not definition_path or not os.path.exists(definition_path):
                        self.logger.error(
                            f"Definition file not found for fixed-width table: {definition_path}"
                        )
                        continue

                    # Parse the field definitions to understand column positions
                    field_definitions = self._parse_fixed_width_definition(
                        definition_path, fixed_width_options
                    )
                    ...
                    field_map = {
                        field["name"]: (field["start"], field["end"]) for field in field_definitions
                    }
                    ...
                    for i, line in enumerate(lines):
                        for composite_config in composite_configs:
                            ...
                            if missing_refs:
                                self.logger.warning(
                                    f"Missing referenced columns: {', '.join(missing_refs)}"
                                )
                            ...
                            try:
                                result = self._evaluate_composite_formula(
                                    original_formula, row_context, i, column_name
                                )
                            except Exception as e:
                                self.logger.warning(
                                    f"Failed to evaluate composite formula for line {i}, column {column_name}: {e}"
                                )
                            ...
                            if len(result) > width:
                                ...
                                self.logger.warning(
                                    f"Truncated result for {column_name}: '{result}' to '{formatted_result}'"
                                )
                            else:
                    ...
                    self.logger.info(
                        f"Saved fixed-width file with processed composite formulas: {output_path}"
                    )
                    ...
                    for col_config in composite_configs:
                        ...
                        for i in range(len(df)):
                            ...
                            try:
                                ...
                                self.logger.warning(
                                    f"Failed to evaluate composite formula for row {i}, column {column_name}: {e}"
                                )
                                self.logger.debug(f"Formula: {original_formula}")
                                result = ""
    ...
    def _parse_fixed_width_definition(
        self, definition_path: str, fixed_width_options: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            with open(definition_path, encoding=encoding) as f:
                ...
                self.logger.debug(
                    f"Column positions: FIELD={name_idx}, START={start_idx}, FINISH={end_idx}"
                )
                ...
                for line in f:
                    ...
                    if len(parts) < 3:
                        self.logger.warning(
                            f"Skipping line {line_count}: too few fields ({len(parts)})"
                        )
                        continue
    ...
    def _preprocess_configuration(self):
        ...
        for table_config in self.config.get("tables", []):
            ...
            if table_config.get("output_type") == "fixed_width":
                ...
                if not definition_path:
                    error_msg = (
                        f"Table '{table_name}': No definition_path specified for fixed-width output"
                    )
                ...
                if not os.path.exists(definition_path):
                    error_msg = (
                        f"Table '{table_name}': Definition file not found: {definition_path}"
                    )
                    self.logger.error(error_msg)
