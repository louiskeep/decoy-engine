...
class MaskingProcessor:
    ...
    def apply_masking_rules(self, df: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        if missing_columns:
            self.logger.warning(
                f"The following columns from masking rules were not found in the data: {', '.join(missing_columns)}"
            )
        ...
        for i, rule in enumerate(self.config["masking_rules"], 1):
            ...
            self.logger.info(
                f"[{i}/{rule_count}] Applying mask to column '{column}' with rule type '{rule['type']}'"
            )
            ...
            if null_count > 0:
                ...
                self.logger.debug(
                    f"Column '{column}' contains {null_count} null values ({null_percent:.1f}%)"
                )
            ...
            if rel_name:
                ...
                self.logger.info(
                    f"Column '{column}' is part of relationship '{rel_name}'. Applying relationship transform."
                )
                df[column] = self.ref_integrity.apply_relationship_transform(
                    df[column], rel_name, rule
                )
            else:
                # Apply regular masking, with optional row-level conditions
                conds = rule.get("conditions")
                if conds:
                    row_mask = self._evaluate_conditions(
                        df, conds, rule.get("condition_logic", "AND")
                    )
    ...
    def _evaluate_conditions(
        ...
        for cond in conditions:
            ...
            if op == "eq":
                ...
                m = s.astype(str).str.contains(str(val), na=False)
            elif op == "not_contains":
                m = ~s.astype(str).str.contains(str(val), na=False)
            elif op == "is_null":
                m = s.isna()
            else:
                m = s.notna()  # is_not_null
            masks.append(m)
