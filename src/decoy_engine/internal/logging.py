...
class ProgressLogger:
        ...
        if self.total > 0:
            ...
            if elapsed_time > 0:
                ...
                if speed > 0:
                    ...
                    self.logger.info(
                        f"{self.message}: {percentage:.1f}% complete ({self.current:,}/{self.total:,}) - {speed_str} - ETA: {eta_str}"
                    )
                else:
                    self.logger.info(
    ...
    def finish(self):
        ...
        if elapsed_time > 0:
            ...
            self.logger.info(
                f"{self.message}: Completed processing {self.current:,} items in {elapsed_time:.1f} seconds ({speed_str})"
            )
        else:
