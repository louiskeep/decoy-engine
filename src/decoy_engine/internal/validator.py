...
class GraphConfigValidator(ConfigValidator):
    ...
    def _validate_nodes(self, nodes: list[dict[str, Any]], kinds: set[str]) -> None:
        ...
        for j, edge in enumerate(edges):
            ...
            if isinstance(src, str) and "." in src:
                ...
                if src not in node_ids:
                    raise ValidationError(f"'from' references unknown node {src!r}", f"{path}.from")

            if dst not in node_ids:
                raise ValidationError(f"'to' references unknown node {dst!r}", f"{path}.to")
    ...
    def _validate_acyclic(self, nodes: list[dict[str, Any]], edges: list[dict[str, Any]]) -> None:
