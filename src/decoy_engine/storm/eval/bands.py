...
source of record.  The qualitative shape observed to date:
    ...
    PROVISIONAL BAND MEASUREMENTS (lgbm-v1; see lightgbm-report.json):
    - high   (>= 0.95): effectively never triggered.  The max calibrated
                         probability observed on the held-out set stays well
                         below 0.95, so the high floor is unreachable with
                         sigmoid calibration at this corpus size.  Empirical
                         guarantee: N/A.
    - review (0.70-0.95): small sample, high accuracy, but too few columns
                           for a tight statistical guarantee.  PROVISIONAL.
    - low    (op-thresh to 0.70): the residual errors are FALSE POSITIVES
                           (``none`` columns over-predicted as a PII type),
                           NOT false negatives -- the model does not miss PII
                           on this fold (the report's ``false_negatives_list``
                           is empty).  PROVISIONAL.

    The model is UNDERCONFIDENT (mean calibration error well above 0; see the
    report's reliability figure): it assigns probabilities in the ~0.5-0.73
    range for columns it classifies correctly nearly 100% of the time.  This is
    expected with CalibratedClassifierCV(sigmoid) on a small calibration set;
    sigmoid calibration cannot produce high-confidence (> 0.9) outputs from
    low-entropy LightGBM leaf scores at this corpus scale.
