    ...
    try:
        ...
        for nid in plan.order:
            ...
            try:
                ...
                records.append(make_export_error_record(nid, kind, exc))
                if log is not None:
                    log.error("graph: node %s failed: %s", node_descriptor(node), exc)
                success = False
                break
            ...
            emit_node_start(log, step_name, descriptor, engine, rows_in_total, node_cfg)
            ...
            try:
                ...
                if isinstance(result, dict) and getattr(op, "OUTPUT_KIND", None) == "split":
                    ...
                    records.append(make_ok_record(nid, kind, elapsed_ms, total_rows, ctx._exports.get(nid)))
                    emit_node_ok(log, step_name, descriptor, elapsed_ms, rows_in_total, total_rows, is_split=True)
                else:
                ...
                # bare free-text.
                records.append(make_error_record(nid, kind, elapsed_ms, translated, ctx._exports.get(nid)))
                # Phase 2 LOGGING_GUIDE §4c: emit_step carries the exception
                # class + the translated message + the canvas node id so the
                # JobLogger can format the spec ERROR line tail.
                emit_node_error(log, step_name, descriptor, elapsed_ms, rows_in_total, translated, nid, exc)
                success = False
                break
            finally:
