...
class FixedWidthHandler(IOHandler):
    ...
    def _parse_definition_file(self, definition_path: str) -> list[dict[str, Any]]:
        ...
        delimiter = self.input_config.get("fixed_width_options", {}).get(
    ...
    def _get_output_field_definitions(self) -> list[dict[str, Any]]:
        ...
        if (
            "fixed_width_options" in self.output_config
            and "definition_path" in self.output_config["fixed_width_options"]
        ):
            output_def_path = self.output_config["fixed_width_options"]["definition_path"]
            if os.path.exists(output_def_path):
                self.logger.debug(
                    f"Using specified output definition file from fixed_width_options: {output_def_path}"
                )
                return self._parse_definition_file(output_def_path)
        ...
        if (
            "fixed_width_options" in self.input_config
            and "definition_path" in self.input_config["fixed_width_options"]
        ):
            input_def_path = self.input_config["fixed_width_options"]["definition_path"]
            if os.path.exists(input_def_path):
                self.logger.debug(
                    f"Using input definition file from fixed_width_options for output: {input_def_path}"
                )
                return self._parse_definition_file(input_def_path)
    ...
    def _format_fixed_width_line(self, row: pd.Series, fields: list[dict[str, Any]]) -> str:
        ...
        global_padding_char = self.output_config.get("fixed_width_options", {}).get(
            "padding_char", " "
        )
        global_alignment = self.output_config.get("fixed_width_options", {}).get(
        ...
        for field in fields:
            ...
            if len(value_str) > width:
                self.logger.warning(
                    f"Value for field '{name}' exceeds defined width. Truncating: '{value_str}'"
                )
    ...
    def _write_definition_file(
        self, fields: list[dict[str, Any]], output_def_path: str, encoding: str = "utf-8"
    ) -> None:
        """
        ...
        # Get delimiter from config or default to comma
        delimiter = self.output_config.get("fixed_width_options", {}).get(
            "definition_delimiter", ","
        )
    ...
    def load_sample(self, sample_rows: int = 5) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.debug(
                f"Loaded sample with {len(df_sample)} rows and {len(df_sample.columns)} columns"
            )
            return df_sample
            ...
            for rule in self.config["masking_rules"]:
                if rule.get("column") == column_name and "fixed_width_options" in rule:
                    ...
                    return {
                        ...
                        "padding_alignment": rule_options.get(
                            "padding_alignment", global_alignment
                        ),
                    }

        # Return global settings
        return {"padding_char": global_padding_char, "padding_alignment": global_alignment}
