...
from decoy_engine.internal.fs import convert_quoting_mode, create_directory_for_file
...
class CSVHandler(IOHandler):
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            df = pd.read_csv(input_path, delimiter=delimiter, encoding=encoding, header=0)
    ...
    def save_data(self, df: pd.DataFrame) -> None:
        ...
        self.logger.debug(
            f"CSV output options: delimiter='{delimiter}', encoding='{encoding}', quoting='{quoting_mode}'"
        )

        try:
            df.to_csv(output_path, index=False, sep=delimiter, encoding=encoding, quoting=quoting)
    ...
    def load_sample(self, sample_rows: int = 5) -> pd.DataFrame:
        ...
        try:
            ...
            df_sample = pd.read_csv(
                input_path, delimiter=delimiter, encoding=encoding, header=0, nrows=sample_rows
            )
    ...
    def get_chunk_iterator(self, chunk_size: int = 100000):
        ...
        try:
            ...
            return pd.read_csv(
                input_path, delimiter=delimiter, encoding=encoding, header=0, chunksize=chunk_size
            )
        except Exception as e:
