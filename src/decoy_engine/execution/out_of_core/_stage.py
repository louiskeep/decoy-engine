...
class MaskedKeyStager:
    ...
    `masked_observed_types` (OOC-B single-read redesign): when the batches
    this stager sees have ALREADY been reconciled onto a stable schema before
    it ever gets them (the payload store's Arrow IPC spill fixes one schema
    for its whole stream, so `_stream_driver.py` reconciles before storing),
    `add`'s own per-batch type detection would only ever see that one fixed
    type and lose the narrower value-derived type a table-level all-null (or
    otherwise degenerate) masked column truly has. Passing the driver's OWN
    pre-reconciliation observation here seeds `observed` with the true types
    instead, and `add` skips re-deriving them from the (already-uniform)
    batches it receives.
    """
