...
# The cohesive dtype byte-cost block (the fixed-width itemsize table, the
# variable-width dtype set, `is_fixed_width_dtype`, and the per-cell
# string-object overhead) lives in `_mem_estimate_dtypes` to keep this module
# under the 600-LOC cap. Re-imported here so external callers that resolve these
# names from `_mem_estimate` (`_mem_estimate_schema`, `out_of_core/
# _spill_estimate`) keep working unchanged.
from decoy_engine.execution._mem_estimate_dtypes import (
    _FIXED_WIDTH_DTYPE_BYTES as _FIXED_WIDTH_DTYPE_BYTES,  # explicit re-export
)
from decoy_engine.execution._mem_estimate_dtypes import (
    _STR_OBJECT_OVERHEAD_BYTES,
    _VARIABLE_WIDTH_DTYPES,
