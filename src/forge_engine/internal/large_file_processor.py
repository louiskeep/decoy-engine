# forge_engine/utils/processor.py
"""
Large file processor for the forge_engine package.
...
class LargeFileProcessor:
    ...
    def process_large_dataset(self, input_path: str, df_schema: pd.DataFrame, processor_func: Callable, 
                              output_path: str = None, description: str = "Processing"):
        """
        ...
        try:
            ...
            self.logger.info(f"File contains {total_rows:,} rows (counted in {count_time:.2f} seconds)")
        ...
        for chunk_num, chunk in enumerate(pd.read_csv(input_path, 
            ...
            if chunk_num % 10 == 0:  # Only log every 10 chunks to avoid excessive logging
                MemoryMonitor.monitor_memory_usage(self.logger, f"Before processing chunk {chunk_num + 1}")
            ...
            if total_rows:
                ...
                self.logger.debug(f"Processed {total_rows_processed:,} rows so far - {rows_per_second:.0f} rows/sec")
        
        if chunk_num % 10 == 0:
            MemoryMonitor.monitor_memory_usage(self.logger, f"After processing chunk {chunk_num + 1}")
