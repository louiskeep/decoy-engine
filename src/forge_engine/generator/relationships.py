# forge_engine/generator/relationships.py
"""
Relationship handling for the forge_engine generator.
...
class RelationshipHandler:
    ...
    def process_self_references(self, table_config: Dict[str, Any], config: Dict[str, Any]) -> None:
        ...
        for rel in self_refs:
            self.logger.info(f"Processing self-reference in table '{table_name}': {rel['column']} -> {rel['reference_column']}")
            ...
            if not os.path.exists(output_path):
                self.logger.warning(f"Cannot process self-reference: Table file not found at {output_path}")
                continue
    ...
    def process_relationships(self, config: Dict[str, Any], reference_data: Dict[str, pd.DataFrame]) -> None:
        ...
        relationships = [rel for rel in config.get('relationships', []) 
                         if rel.get('type') != 'self_reference']
    ...
    def _process_foreign_key_relationship(self, relationship: Dict[str, Any], 
        ...
        self.logger.info(f"Processing foreign key: {source_table}.{source_column} -> {target_table}.{target_column}")
        
        # Check if tables are available
        if source_table not in reference_data or target_table not in reference_data:
            self.logger.warning(f"One or both tables ({source_table}, {target_table}) not found in reference data")
            return
        ...
        if source_column not in source_df.columns or target_column not in target_df.columns:
            self.logger.warning(f"One or both columns ({source_column}, {target_column}) not found in tables")
            return
            ...
            for i in range(len(df)):
                ...
                if distribution == 'random':
                    ...
                    if not weights or len(weights) != len(target_values):
                        ...
                        self.logger.warning(f"Weights for relationship '{rel_name}' do not sum to 1.0 (sum: {sum(weights):.6f}). Using equal weights instead.")
                        weights = None
    ...
    def _process_many_to_many_relationship(self, relationship: Dict[str, Any], 
        ...
        if not all([junction_table, left_table, left_column, right_table, right_column]):
            self.logger.warning("Incomplete many-to-many relationship definition. Required: junction_table, left_table, left_column, right_table, right_column")
            return
            
        self.logger.info(f"Processing many-to-many relationship: {left_table} <- {junction_table} -> {right_table}")
        ...
        if left_column not in junction_df.columns:
            self.logger.warning(f"Left column '{left_column}' not found in junction table '{junction_table}'")
            return
        if right_column not in junction_df.columns:
            self.logger.warning(f"Right column '{right_column}' not found in junction table '{junction_table}'")
            return
        ...
        if not left_values or not right_values:
            self.logger.warning(f"No values found in left table '{left_table}' column '{left_column}' or right table '{right_table}' column '{right_column}'")
            return
        ...
        if left_cardinality == 'one' and right_cardinality == 'one':
            ...
            for left_val in left_values:
                ...
                num_connections = min(random.randint(1, min(5, len(right_values))), len(right_values))
            ...
            if max_relationships:
                ...
                num_relationships = min(int(total_possible * random.uniform(0.2, 0.5)), total_possible)
        ...
        if len(relationships) > junction_rows:
            self.logger.warning(f"Generated {len(relationships)} relationships but junction table only has {junction_rows} rows. Truncating relationships.")
