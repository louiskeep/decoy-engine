...
| Unified-slice production-readiness lane (Task 4.5, engine-ready; caller activation Task 4.6) | `src/decoy_engine/execution/_unified_slice.py` (routing entry point, diversion after layer-2 FK routing checks); `src/decoy_engine/execution/_unified_slice_admission.py` (D3 conservative total allowlist over resident-type domain); `src/decoy_engine/execution/physical/_live_inputs.py` (`build_live_physical_plan_inputs`, constructs PhysicalPlanInputs from run_pipeline's already-computed profile + plan facts, no re-profiling). Routes via `run_pipeline(unified_slice_enabled=True)` (default off) for bounded non-FK single-table masks on already-resident Parquet pa.Table sources; runs 4.3 physical plan through 4.4 coordinator; returns ExecutionResult identical to pandas path. Production activation (e.g. caller-facing flag) is Task 4.6. |
