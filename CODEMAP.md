...
| `src/decoy_engine/execution/out_of_core/` | Opt-in batch-streaming out-of-core FK masking (SC1). Entry point: `run_fk_out_of_core`. Streams tables through bounded batch passes with on-disk DuckDB spill. Modules: `_runner.py` (orchestrator), `_batch_join.py` (batch FK joining), `_mask.py` (batch column masking), `_relation.py` (parent-key relation staging), `_source.py` (lazy reader), `_emit.py` (batch-to-sink emission), `_join.py` (FK resolution), `_stage.py` (staging logic), `_compat.py` (DuckDB capability check), `_budget.py` (temp disk budget), `_duckdb.py` (backend bridge). |
