...
| `src/decoy_engine/execution/_pandas_adapter.py` | `PandasExecutionAdapter` (the only masking substrate) |
...
| Add masking strategy | Implement `StrategyHandler` under `execution/_strategies/`, wire into the Pandas adapter dispatch, add unit + golden coverage |
...
| Substrate selection respects env | `DECOY_SUBSTRATE=pandas` (the only valid masking substrate) resolves per `_substrate.py`; any other value raises `invalid_substrate` |
