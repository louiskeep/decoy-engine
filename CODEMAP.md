...
| `src/decoy_engine/execution/_pipeline_routing_signals.py` | Route-admission size and OOM-prevention signals (SC7b: `largest_mask_table_rows_from_profile`, per-table reconciliation of resident vs. profile row counts, lazy-path admission gating); re-exported from `_pipeline_routing` to hold the 600-LOC orchestration cap |
...
| `src/decoy_engine/execution/` | `ExecutionAdapter` Protocol, `PandasExecutionAdapter`, `select_execution_adapter`, `_strategies/` (column-strategy handlers), `polars/` (Polars adapter), `_sequential.py` (bounded-memory masked table-by-table FK execution in FK-topological order; S2 is the default path for eligible jobs routed by `run_pipeline`), `out_of_core/` (Phase-4 batch-streaming out-of-core route with DuckDB spill, accessed via `run_fk_out_of_core`; auto-selected per-table by parent-key row count via `decide_route` in `_route_policy.py`, default threshold 2,000,000 rows; routes between the fast-path `_batch_join` route and the new `_stream_driver` reorder route), `_native_route.py` + `_native_route_exec.py` (Q3 slice 1 single-pass streaming native lane for passthrough/redact/truncate on utf8 columns; Q3 slice 2 widened to integer, boolean, and timestamp columns with preflight read plus execution read, routes via `run_pipeline(native_route_enabled=True)`), `_unified_slice.py` + `_unified_slice_admission.py` (Task 4.5 production lane, DEFAULT-ON per-run flag (activated 2026-09-20), routes bounded non-FK single-table masks on already-resident Parquet sources via the physical plan + coordinator; returns ExecutionResult identical to pandas path; an inert worker sink is admitted), `_transactional_sink.py` (all-or-nothing sink protocol + Parquet reference implementation), `_planner.py` (execution-mode planner), `_chunked.py` (`run_mask_pipeline_chunked`, `concat_masked_chunks`; backs the `auto_chunk` route), `_pipeline_routing.py` (composes the sequential + auto-chunk routing decisions) |
| `src/decoy_engine/execution/_transactional_sink.py` | `TransactionalSink` four-method protocol (write/write_batches/commit/abort; `abort` is best-effort and must not raise) and `ParquetTransactionalSink` file-based reference implementation. `write(table, data)` accepts a full Arrow table; `write_batches(table, batches, schema)` streams batches without materializing the whole table (used by out-of-core and chunked routes). `commit()` publishes via a single atomic POSIX directory rename (`os.replace`, POSIX rename(2)); either every Parquet file lands at once or nothing is published (visibility-atomicity, not fsync durability). A pre-existing non-empty target causes commit to fail closed. Both are exported from `decoy_engine.execution`. Used by `run_sequential` (`_sequential.py`) and `run_fk_out_of_core` (SC1) for FK jobs that require all-or-nothing output. |
| `src/decoy_engine/execution/out_of_core/` | Opt-in batch-streaming out-of-core FK masking (SC1, widened SC3/SC4). Entry point: `run_fk_out_of_core`. Streams tables through bounded batch passes with on-disk DuckDB spill. Modules: `_runner.py` (orchestrator), `_batch_join.py` (batch FK joining), `_mask.py` (dispatcher), `_mask_group_b.py` (SC3 Group b strategies: fpe, text_redact, categorical), `_mask_group_c.py` (SC4 Group c strategies: text_mask, code_set, bucket_perturb), `_relation.py` (parent-key relation staging), `_source.py` (lazy reader), `_emit.py` (batch-to-sink emission), `_join.py` (FK resolution), `_stage.py` (staging logic), `_compat.py` (compatibility gate for supported strategies), `_budget.py` (temp disk budget), `_duckdb.py` (backend bridge). Supported payload strategies: SC1 initial (hash, redact, truncate, passthrough), SC3 Group (b) (fpe, text_redact, categorical deterministic), SC4 Group (c) (text_mask unconditional; code_set mask-only; bucket_perturb explicit-date_format-only). FK parent-key surface remains SC1 initial only. |
| `src/decoy_engine/execution/physical/` | Physical-execution adapter seam (Tasks 4.2-4.5 of execution-consolidation program). Modules: `_types` (seam identifiers), `_context` + `_shadow_context` (per-invocation identity carriers), `_capabilities` (frozen capability table), `_protocols` (BatchOperator family protocols, TableDriver protocol), `drivers/` (six delegating adapters), `_inputs` + `_snapshot` + `_live_inputs` (PhysicalPlanInputs builders: `_snapshot.py` re-derives for shadow testing; `_live_inputs.py` builds from run_pipeline facts for Task 4.5 production lane), `_plan` + `_compiler` + `_reasons` (Task 4.3 physical-plan builder), `_shadow_*` modules (Task 4.4 shadow coordinator + operators + snapshot + difference codes), `_activation` (Task 4.5 production-readiness activation overlay). The seam is reach by a single production entry point: `execution/_unified_slice.py` (Task 4.5, default-on since 2026-09-20, routes through the physical plan + coordinator for bounded non-FK single-table masks); every other route/coordinator/executor module stays disconnected (enforced by `tests/sentry/test_physical_seam_disconnection.py`). |
| `src/decoy_engine/kernel/` | Backend-neutral deterministic masking kernel (SC1). Shared Arrow-native scalar masking logic: `hash_array`, `redact_array`, `truncate_array`, `passthrough_array` (per-value functions independent of batch boundary or row position). Also exports: `canonicalize_derive_source` (source normalization), `encode_int` (integer encoding). Used by out-of-core batch logic and chunked-FK gate. |
| `src/decoy_engine/execution/_chunked_fk.py` | FK self-masking gate for chunked execution (S3 / SC1). Validates strategy eligibility (`CHUNK_SAFE_STRATEGIES`: hash, fpe, redact, truncate, text_redact, date_shift, bucketize, passthrough). Enforces namespace requirements at compile time for namespace-requiring strategies (hash, fpe, date_shift). |
...
| `src/decoy_engine/storm/` | Profiling and detectors; `eval/` (labeled fixtures + regex-baseline recognition harness + ML measurement substrate) and `features/` (deterministic per-column feature builder) are off-run-path field-recognition tooling (BF2 / ML0); `eval/fixtures.py` holds the base 5-fixture corpus + checksum primitives, `eval/corpus.py` (MLF-4) holds the expanded ~2958-column training corpus + OOD/cryptic-header benchmark generators (`build_extended_fixtures` / `build_ood_fixtures` / `build_cryptic_fixtures`); `eval/split.py` + `eval/bands.py` are scaffolding for ML2.2; `model_pack/` holds the LightGBM pack loader + featurizer + ML3.1 classification function + ML3.2 HMAC provenance signing |
...
| Out-of-core batch-streaming FK execution (Phase-4 opt-in) | `src/decoy_engine/execution/out_of_core/` (`run_fk_out_of_core` entry point); per-table route chosen by `decide_route` in `_route_policy.py` based on parent-key row count (default threshold 2,000,000); routes between `_batch_join.py` (fast-path small-parent) and `_stream_driver.py` (reorder route for large-parent high-fan-in tables); uses DuckDB spill and staged parent-key relations for bounded batch passes |
| Backend-neutral deterministic masking kernel (scalar hash/redact/truncate/passthrough) | `src/decoy_engine/kernel/` (`hash_array`, `redact_array`, `truncate_array`, `passthrough_array`, `canonicalize_derive_source`, `encode_int`) |
| FK self-masking gate for chunked/batch execution (S3, SC1) | `src/decoy_engine/execution/_chunked_fk.py` (strategy eligibility: `CHUNK_SAFE_STRATEGIES`; namespace-requirement enforcement) |
...
| Streaming native lane (Q3 slices 1-2: slice 1 single-pass for utf8, slice 2 widened with two-read preflight for integer/boolean/timestamp; opt-in, default off) | `src/decoy_engine/execution/_native_route.py` (`static_candidacy`, `peek_and_admit`, `maybe_run_native_route` routing decision); `src/decoy_engine/execution/_native_route_exec.py` (`try_native_route` execution entry point, batch masking, ledger validation, transactional publication); `src/decoy_engine/execution/_native_route_preflight.py` (global column state classification, admission matrix resolver); `src/decoy_engine/execution/_native_route_digest.py` (source-snapshot identity codec); routes via `run_pipeline(native_route_enabled=True)` for single-table non-FK pure-mask jobs using passthrough/redact/truncate on utf8, integer, boolean, and timestamp columns; never materializes the source (LazySource streams in 50k-row batches). Eligibility gates run before source read; any feature or shape unsupported (FK, multi-table, validators, fidelity-report, quarantine, non-supported columns, vault, non-LazySource) reroutes to the oracle. Mutation grading 94.22% (766/813 killed) at `docs/quality/mutation-ledgers/execution_native_route.md`. |
| Unified-slice production lane (Task 4.5, activated as production default 2026-09-20) | `src/decoy_engine/execution/_unified_slice.py` (routing entry point, diversion after layer-2 FK routing checks); `src/decoy_engine/execution/_unified_slice_admission.py` (D3 conservative total allowlist over resident-type domain); `src/decoy_engine/execution/physical/_live_inputs.py` (`build_live_physical_plan_inputs`, constructs PhysicalPlanInputs from run_pipeline's already-computed profile + plan facts, no re-profiling). Routes via `run_pipeline` (`unified_slice_enabled` default True since 2026-09-20; pass False to force legacy) for bounded non-FK single-table masks on already-resident Parquet pa.Table sources; runs 4.3 physical plan through 4.4 coordinator; returns ExecutionResult identical to pandas path. The admission predicate is the safety gate; a non-admissible shape falls through to the legacy route. |
| Physical-execution adapter seam (Tasks 4.2-4.5) | `src/decoy_engine/execution/physical/` package: seam identifiers (`_types`), invocation carriers (`_context`, `_shadow_context`), capability table (`_capabilities`), protocols (`_protocols`, `drivers/`), input builders (`_inputs`, `_snapshot`, `_live_inputs`), Task 4.3 compiler (`_compiler`, `_plan`, `_reasons`), Task 4.4 shadow coordinator (`_shadow_*`), Task 4.5 activation (`_activation`). See package docstring and `tests/sentry/test_physical_seam_disconnection.py` for disconnection proof: only `execution/_unified_slice.py` (Task 4.5) is production-authorized to import the seam; every other route stays disconnected. |
| Out-of-core route selection (Phase 4, Task 7) | `src/decoy_engine/execution/out_of_core/_route_policy.py::decide_route` (per-table sink-path route dispatch by parent-key row count; default threshold `REORDER_PARENT_KEY_THRESHOLD=2,000,000`); also exports topology helpers `_table_order` and `_edge_indexes` |
| Out-of-core reorder driver (Phase 4, Task 6) | `src/decoy_engine/execution/out_of_core/_stream_driver.py::stream_table` (entry point for high-fan-in large-parent tables); `src/decoy_engine/execution/out_of_core/_external_sort.py` (stable external-sort module with two-file spill, produces parent-key-sorted child batches); `src/decoy_engine/execution/out_of_core/_stream_join.py` (streaming-join core logic, decomposed from `_runner.py` for size); `src/decoy_engine/execution/out_of_core/_stream_join_cursors.py` (cursor-tracking for multi-parent co-residency) |
| Out-of-core masking and relation staging (Phase 4) | `src/decoy_engine/execution/out_of_core/_mask.py`, `_mask_group_b.py`, `_mask_group_c.py` (payload strategy dispatch); `src/decoy_engine/execution/out_of_core/_relation.py` (parent-key relation staging with Phase-4 split-dedup for bounded-parent cardinality); `src/decoy_engine/execution/out_of_core/_batch_join.py` (small-parent fast-path, complement to reorder route) |
