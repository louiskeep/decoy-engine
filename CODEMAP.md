...
| Single-pass streaming native lane (Q3 slice 1, opt-in, default off) | `src/decoy_engine/execution/_native_route.py` (`static_candidacy`, `peek_and_admit`, `maybe_run_native_route` routing decision); `src/decoy_engine/execution/_native_route_exec.py` (`try_native_route` execution entry point, batch masking, ledger validation, transactional publication); routes via `run_pipeline(native_route_enabled=True)` for single-table non-FK pure-mask jobs using passthrough/redact/truncate strategies on utf8 string columns only; never materializes the source (LazySource streams in 50k-row batches). Eligibility gates run before source read; any feature or shape unsupported (FK, multi-table, validators, fidelity-report, quarantine, non-string columns, vault, non-LazySource) reroutes to the oracle. Mutation grading 94.22% (766/813 killed) at `docs/quality/mutation-ledgers/execution_native_route.md`. |
