...
def test_truncate_invalid_length_falls_back(mock_logger):
    ...
    for bad in (None, 0, -3, "three", True, 1.5):
        out = strat.apply(column, {"column": "x", "type": "truncate", "length": bad})
...
def test_bucketize_decade_lower(mock_logger):
    ...
    out = strat.apply(column, {"column": "age", "type": "bucketize", "preset": "by_decade"})
...
def test_bucketize_extended_presets(mock_logger):
    ...
    cases = [
        ...
        ("by_ten_thousand", pd.Series([0, 9999, 10000, 75000]), ["0", "0", "10000", "70000"]),
    ]
    for preset, column, expected in cases:
        out = strat.apply(
            column,
            {"column": "v", "type": "bucketize", "preset": preset},
        )
...
def test_bucketize_custom_width_float(mock_logger):
    ...
    out = strat.apply(column, {"column": "price", "type": "bucketize", "width": 0.5})
...
def test_bucketize_negative_values(mock_logger):
    ...
    out = strat.apply(column, {"column": "x", "type": "bucketize", "width": 10})
