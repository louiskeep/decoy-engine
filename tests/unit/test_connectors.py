...
def test_csv_handler(sample_csv_file, mock_logger, tmp_path):
    ...
    output_config = {
        "type": "csv",
        "path": output_path,
        "csv_options": {"delimiter": ",", "encoding": "utf-8", "quoting": "minimal"},
    }
...
def test_fixed_width_handler(sample_fixed_width_file, mock_logger, tmp_path):
    ...
    input_config = {
        ...
        "fixed_width_options": {"encoding": "utf-8", "definition_delimiter": ","},
    }
    ...
    # CSV round-trip does not preserve pandas dtype metadata (object vs StringDtype);
    # compare values only.
...
def test_io_handler_factory(sample_csv_file, sample_fixed_width_file, mock_logger):
    ...
    # CSV handler
    input_config_csv = {"type": "csv", "path": sample_csv_file}

    handler_csv = create_io_handler(
        input_config_csv, {"type": "csv", "path": "dummy.csv"}, mock_logger
    )
    ...
    handler_fw = create_io_handler(
        input_config_fw, {"type": "csv", "path": "dummy.csv"}, mock_logger
    )
    assert isinstance(handler_fw, FixedWidthHandler)

    # Invalid type
    input_config_invalid = {"type": "invalid", "path": "invalid.txt"}
