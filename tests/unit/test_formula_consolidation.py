...
class TestMaskFormulaSingleEvaluator:
    ...
    def test_basic_expression(self):
        ...
        out = s.apply(pd.Series(['hello', 'world']), {
            'column': 'x', 'type': 'formula',
            'formula': 'value.upper()',
        })
    ...
    def test_fstring_template(self):
        ...
        # themselves with `f"..."`.
        s = FormulaStrategy(logger=_LOG)
        out = s.apply(pd.Series(['alice', 'bob']), {
    ...
    def test_nulls_preserved(self):
        s = FormulaStrategy(logger=_LOG)
        out = s.apply(pd.Series(['a', None, 'b']), {
            'column': 'x', 'type': 'formula',
            'formula': 'value.upper()',
        })
        assert out.iloc[0] == 'A'
...
class TestGenerateInlineFormula:
    ...
    def test_different_seeds_yield_different_output(self):
        ...
        assert any(a != b for a, b in zip(out1, out2))
