"""Static reference-vector test for the v3 envelope.
...
the reference vector in the same PR. Two prior bumps did exactly that:
  ...
  v2 -> v3: QA walks/generators F3 / PO Q-F3=b (2026-06-01). Vectorised
...
The vector below is recomputed against the v3 envelope; derivation
...
# Hand-computed reference vector against the published v3 envelope:
#
#   seed       = b"\x00" * 8
...
#   HMAC_input = (
#       bytes([SEED_PROTOCOL_VERSION])              # 0x03 (v3 envelope)
...
#
# Computed value (regenerated at the v2 -> v3 bump for QA walks/gen F3
# null-injection vectorisation, PO Q-F3=b 2026-06-01; pinned thereafter):
EXPECTED_HEX_V3 = "1fef9981087b99fb8a7c30bf4cf085346bf635a12e6f1ac7e04e48dbbf2e02b8"
...
@pytest.mark.golden
class TestDeriveReferenceVectorV3:
    def test_seed_protocol_version_is_three(self) -> None:
    ...
    def test_v3_envelope_matches_reference_vector(self) -> None:
        ...
        assert actual == EXPECTED_HEX_V3, (
            f"v3 envelope drift: expected {EXPECTED_HEX_V3}, got {actual}. "
            "Either the envelope shape changed (HKDF salt, length-prefix, "
            "version byte, byte-order) or SEED_PROTOCOL_VERSION needs bumping "
            "with a same-PR update to EXPECTED_HEX_V3."
        )
