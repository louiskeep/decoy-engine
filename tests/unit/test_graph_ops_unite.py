"""Unit tests for the `unite` op and the multi-input arity error hint."""

import pandas as pd
import pytest

from decoy_engine.graph.ops import OPS, unite
from decoy_engine.graph.ops._base import OpError
from decoy_engine.internal.validator import GraphConfigValidator, ValidationError


class TestRegistry:
    def test_unite_kind_registered(self):
        assert "unite" in OPS, "'unite' not registered in graph.ops.OPS"

    def test_unite_arity_is_two_or_more(self):
        assert unite.INPUT_ARITY == (2, None)


class TestUnitePositional:
    def test_concat_distinct_columns(self):
        df1 = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
        df2 = pd.DataFrame({"c": [5, 6], "d": [7, 8]})
        out = unite.apply([df1, df2], {}, None)
    ...
    def test_three_way_positional(self):
        ...
        out = unite.apply([df1, df2, df3], {}, None)
...
class TestUniteKeyed:
    ...
    def test_outer_join_keeps_all(self):
        ...
        df2 = pd.DataFrame({"id": [2, 3], "age": [30, 35]})
        out = unite.apply([df1, df2], {"on": ["id"], "join_type": "outer"}, None)
        assert sorted(out["id"].tolist()) == [1, 2, 3]
    ...
    def test_three_way_keyed(self):
        ...
        out = unite.apply([df1, df2, df3], {"on": ["id"]}, None)
    ...
    def test_missing_on_column_raises_op_error(self):
        ...
        with pytest.raises(OpError, match="missing 'on'"):
            unite.apply([df1, df2], {"on": ["id"]}, None)


class TestUniteValidation:
    ...
    def test_validate_accepts_minimal_config(self):
        unite.validate_config({})
        unite.validate_config({"on": ["id"]})
        unite.validate_config(
            {"on": ["id"], "join_type": "outer", "suffixes": ["_x", "_y"]}
        )


class TestArityHintsAtUnite:
    """The graph validator nudges users toward `unite` when a single-input
    op (mask/gen/derive/...) is wired with too many incoming edges."""

    def _validate(self, cfg):
        import logging

        log = logging.getLogger("test_unite_arity")
        if not log.handlers:
            log.addHandler(logging.NullHandler())
        GraphConfigValidator(log).validate(cfg)

    def test_two_edges_into_mask_hints_at_unite(self):
        cfg = {
            "mode": "graph",
            "nodes": [
        ...
        with pytest.raises(ValidationError) as exc:
            self._validate(cfg)
        assert "unite" in str(exc.value)
        assert "at most 1" in str(exc.value)

    def test_two_edges_into_generate_hints_at_unite(self):
        cfg = {
            "mode": "graph",
            "nodes": [
                {"id": "s1", "kind": "source.file", "config": {"path": "/tmp/a"}},
...
class TestNodeNameField:
    ...
    def _validate(self, cfg):
        ...
        log = logging.getLogger("test_unite_name")
        if not log.handlers:
            log.addHandler(logging.NullHandler())
        GraphConfigValidator(log).validate(cfg)
