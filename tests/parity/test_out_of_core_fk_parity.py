...
# ---------------------------------------------------------------------------
# High fan-in (one child, several INCOMING edges) parity
#
# The multiplied-pinned-relation shape fix#1 (single-streaming-join) targets:
# one child table carries several distinct FK edges at once, so `_stream_table`
# opens several joiners co-live over one child stream. Distinct from fan-OUT
# (one parent, many child tables) above and NOT the rejected multi-parent shape
# (two parents for the SAME child column tuple). Pinned green on pre-change code
# so the three-phase restructure is gated on it.
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("policy", list(OrphanPolicy))
@pytest.mark.parametrize("strategy", ["hash", "redact", "truncate", "passthrough"])
def test_high_fan_in_two_single_col_edges_parity(strategy: str, policy: OrphanPolicy) -> None:
    """One child with two INCOMING single-column FK edges to two parents.

    Both FK columns are rewritten over one child stream (two co-live joiners),
    with orphans/nulls in each column crossing batch boundaries, so any
    per-edge key-source or ordering slip in the restructure diverges here.
    """
    cust_seed = _seed_for(strategy, "ns_cust")
    prod_seed = _seed_for(strategy, "ns_prod")
    customers = pa.table({"customer_id": pa.array(["c0", "c1", "c2"], type=pa.string())})
    products = pa.table({"product_id": pa.array(["p0", "p1"], type=pa.string())})
    # 8 rows; FAIL needs zero orphans, others exercise orphans + nulls in each
    # FK column, offset so the two columns' orphan rows do not coincide.
    if policy is OrphanPolicy.FAIL:
        cust_refs = ["c1", "c0", None, "c2", "c1", "c0", None, "c2"]
        prod_refs = ["p0", None, "p1", "p0", None, "p1", "p0", "p1"]
    else:
        cust_refs = ["c1", "missA", None, "c2", "c1", "missB", None, "c2"]
        prod_refs = ["p0", None, "missP", "p0", None, "p1", "missQ", "p1"]
    orders = pa.table(
        {
            "customer_id": pa.array(cust_refs, type=pa.string()),
            "product_id": pa.array(prod_refs, type=pa.string()),
            "amount": pa.array(list(range(8)), type=pa.int64()),
        }
    )
    plan = _plan(
        (
            ("customers", TableSeed(per_column=(("customer_id", cust_seed),), per_group=())),
            ("products", TableSeed(per_column=(("product_id", prod_seed),), per_group=())),
            (
                "orders",
                TableSeed(
                    per_column=(("customer_id", cust_seed), ("product_id", prod_seed)),
                    per_group=(),
                ),
            ),
        )
    ...
    _assert_parity_or_faithful_rejection(
        plan,
        {"customers": customers, "products": products, "orders": orders},
        graph,
        f"high_fan_in/{strategy}/{policy.name}",
    )


@pytest.mark.parametrize("policy", list(OrphanPolicy))
def test_high_fan_in_composite_and_single_edge_parity(policy: OrphanPolicy) -> None:
    """Mixed fan-in: one child carries a COMPOSITE FK edge and a single-column
    FK edge at once, so a composite joiner and a scalar joiner are co-live over
    one stream. The composite key uses a `composite_fk_group` (the oracle-parity
    shape); the partial-null composite row is treated as fully null by both.
    """
    ns_acct = "ns_acct"
    country_seed = _seed_for("hash", "ns_country")
    account_seed = _seed_for("hash", "ns_account")
    region_seed = _seed_for("passthrough", "ns_region")
    accounts = pa.table({"country": ["US", "CA", "US"], "account_id": ["a0", "a1", "a2"]})
    regions = pa.table({"region_id": ["r0", "r1"]})
    if policy is OrphanPolicy.FAIL:
        countries = ["CA", "US", None, "US", "CA", "US"]
        accts = ["a1", "a0", "a0", "a2", "a1", "a0"]
        region_refs = ["r0", "r1", None, "r0", "r1", "r0"]
    else:
        countries = ["CA", "US", None, "US", "CA", "US"]
        accts = ["a1", "a0", "a0", "missAcct", "a1", "a0"]
        region_refs = ["r0", "missR", None, "r0", "r1", "r0"]
    txns = pa.table(
        {
            "country": pa.array(countries, type=pa.string()),
            "account_id": pa.array(accts, type=pa.string()),
            "region_id": pa.array(region_refs, type=pa.string()),
        }
    )
    plan = _plan(
        (
            (
                "accounts",
                TableSeed(
                    per_column=(("country", country_seed), ("account_id", account_seed)),
                    per_group=(),
                ),
            ),
            ("regions", TableSeed(per_column=(("region_id", region_seed),), per_group=())),
            (
                "txns",
                TableSeed(
                    per_column=(("region_id", region_seed),),
                    per_group=(
                        (
                            "account_id__country",
                            GroupSeed(
    ...
    _assert_parity_or_faithful_rejection(
        plan,
        {"accounts": accounts, "regions": regions, "txns": txns},
        graph,
        f"high_fan_in_composite/{policy.name}",
    )
