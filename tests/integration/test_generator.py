...
def test_generator_integration(sample_generator_config, tmp_path, mock_logger):
    ...
    with open(sample_generator_config) as f:
    ...
    DataGenerator(updated_config_path, mock_logger)
...
def test_generator_composite_formula(tmp_path, mock_logger):
    ...
    config = {
        "generator_settings": {"seed": 42, "output_directory": str(tmp_path)},
        "tables": [
            {
                "name": "people",
                "rows": 5,
    ...
    expected_usernames = (
        generated["first_name"].str.lower() + "_" + generated["last_name"].str.lower()
    )
...
def test_generator_fixed_width(tmp_path, mock_logger):
        ...
        "tables": [
            {
                ...
                "fixed_width_options": {"encoding": "utf-8", "definition_path": def_path},
                "columns": [
    ...
    assert set(generated_data["department"]).issubset({"HR", "IT", "Finance", "Marketing", "Sales"})
